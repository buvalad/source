...
void CGrannyMesh::LoadIndices(void* dstBaseIndices, bool use32bit)
{
	const granny_mesh* pgrnMesh = GetGrannyMeshPointer();

	if (use32bit)
	{
		DWORD* dstIndices = ((DWORD*)dstBaseIndices) + m_idxBasePos;
		GrannyCopyMeshIndices(pgrnMesh, sizeof(DWORD), dstIndices);
	}
	else
	{
