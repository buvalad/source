...
bool CGrannyModel::LoadIndices()
{
	...
	bool use32bit = (m_idxCount > 50000);  // Sau după alte criterii

	if (!m_idxBuf.Create(m_idxCount, use32bit ? D3DFMT_INDEX32 : D3DFMT_INDEX16))
		return false;
	...
	{
		CGrannyMesh& rMesh = m_meshs[m];
		rMesh.LoadIndices(indices, use32bit);
	}
...
bool CGrannyModel::LoadMeshs()
{
	...
	for (int m = 0; m < meshCount; ++m)
	{
		...
		if (vtxPos > 1000000000 || idxPos > 1000000000) // sau cât vrei
		{
			TraceError("Model prea mare: %d vertices, %d indices", vtxPos, idxPos);
			// return false; // opțional dacă vrei doar log
		}
	} // <-- ACEASTA ERA ACOLOADA CARE LIPSEA!
