...
// ACEASTA ESTE FUNCTIA MODIFICATA PENTRU SUPORT INDEX16/INDEX32:
bool CSnowEnvironment::__CreateGeometry()
{
	// Decide formatul index bufferului în funcție de câți vertecși ai
	if (m_dwParticleMaxNum * 4 > 65535)
		m_indexFormat = D3DFMT_INDEX32;
	else
		m_indexFormat = D3DFMT_INDEX16;

	UINT indexSize = (m_indexFormat == D3DFMT_INDEX32) ? sizeof(DWORD) : sizeof(WORD);
	int numIndices = m_dwParticleMaxNum * 6;

	// Creează vertex buffer-ul (la fel ca înainte)
	...
	// Creează index buffer-ul în formatul corect
	if (FAILED(ms_lpd3dDevice->CreateIndexBuffer(indexSize * numIndices,
		D3DUSAGE_WRITEONLY,
		m_indexFormat,
		D3DPOOL_MANAGED,
		&m_pIB)))
	...
	// Scrie indicii în buffer, în funcție de format
	if (m_indexFormat == D3DFMT_INDEX32)
	{
		DWORD* dstIndices;
		if (FAILED(m_pIB->Lock(0, numIndices * sizeof(DWORD), (BYTE**)&dstIndices, 0)))
			return false;
		const DWORD c_awFillRectIndices[6] = { 0, 2, 1, 2, 3, 1, };
	...
	{
		WORD* dstIndices;
		if (FAILED(m_pIB->Lock(0, numIndices * sizeof(WORD), (BYTE**)&dstIndices, 0)))
			return false;
...
void CSnowEnvironment::__Initialize()
{
	...
	m_indexFormat = D3DFMT_INDEX16; // Default, se resetează corect în __CreateGeometry()
